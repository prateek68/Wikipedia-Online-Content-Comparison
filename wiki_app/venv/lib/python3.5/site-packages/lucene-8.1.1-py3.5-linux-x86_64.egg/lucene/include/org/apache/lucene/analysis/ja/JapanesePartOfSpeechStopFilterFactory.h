#ifndef org_apache_lucene_analysis_ja_JapanesePartOfSpeechStopFilterFactory_H
#define org_apache_lucene_analysis_ja_JapanesePartOfSpeechStopFilterFactory_H

#include "org/apache/lucene/analysis/util/TokenFilterFactory.h"

namespace java {
  namespace io {
    class IOException;
  }
  namespace util {
    class Map;
  }
  namespace lang {
    class String;
    class Class;
  }
}
namespace org {
  namespace apache {
    namespace lucene {
      namespace analysis {
        namespace util {
          class ResourceLoader;
          class ResourceLoaderAware;
        }
        class TokenStream;
      }
    }
  }
}
template<class T> class JArray;

namespace org {
  namespace apache {
    namespace lucene {
      namespace analysis {
        namespace ja {

          class JapanesePartOfSpeechStopFilterFactory : public ::org::apache::lucene::analysis::util::TokenFilterFactory {
           public:
            enum {
              mid_init$_c0bf7b502c3283d5,
              mid_create_12466f62672d3a0b,
              mid_inform_ffdd95be4370257e,
              max_mid
            };

            static ::java::lang::Class *class$;
            static jmethodID *mids$;
            static bool live$;
            static jclass initializeClass(bool);

            explicit JapanesePartOfSpeechStopFilterFactory(jobject obj) : ::org::apache::lucene::analysis::util::TokenFilterFactory(obj) {
              if (obj != NULL && mids$ == NULL)
                env->getClass(initializeClass);
            }
            JapanesePartOfSpeechStopFilterFactory(const JapanesePartOfSpeechStopFilterFactory& obj) : ::org::apache::lucene::analysis::util::TokenFilterFactory(obj) {}

            JapanesePartOfSpeechStopFilterFactory(const ::java::util::Map &);

            ::org::apache::lucene::analysis::TokenStream create(const ::org::apache::lucene::analysis::TokenStream &) const;
            void inform(const ::org::apache::lucene::analysis::util::ResourceLoader &) const;
          };
        }
      }
    }
  }
}

#include <Python.h>

namespace org {
  namespace apache {
    namespace lucene {
      namespace analysis {
        namespace ja {
          extern PyType_Def PY_TYPE_DEF(JapanesePartOfSpeechStopFilterFactory);
          extern PyTypeObject *PY_TYPE(JapanesePartOfSpeechStopFilterFactory);

          class t_JapanesePartOfSpeechStopFilterFactory {
          public:
            PyObject_HEAD
            JapanesePartOfSpeechStopFilterFactory object;
            static PyObject *wrap_Object(const JapanesePartOfSpeechStopFilterFactory&);
            static PyObject *wrap_jobject(const jobject&);
            static void install(PyObject *module);
            static void initialize(PyObject *module);
          };
        }
      }
    }
  }
}

#endif
